        ...
        public IActionResult Upload(IFormFile file)
        {
            ...
            if (file.Length > 0)
            {
                using (var pck = new OfficeOpenXml.ExcelPackage())
                {
                    ...
                    {
                        // Not enough rows
                        ModelState.AddModelError("patient", $"Patient worksheet doesn't have enough rows {pck.Workbook.Worksheets[0].Dimension.Rows}");
                    }
                    if (pck.Workbook.Worksheets[0].Dimension.Rows > 1)
                    {
                        // Not enough columns to include header and data
                        if (pck.Workbook.Worksheets[0].Dimension.Columns < 6)
                        {
                            // Not enough columns
                            ModelState.AddModelError("patient", $"Patient worksheet doesn't have enough columns {pck.Workbook.Worksheets[0].Dimension.Columns}");
                        }
                        else
                        {
                                     ...
                                     i++)
                            {
                                ...
                                if (pck.Workbook.Worksheets[0].Cells[i, 1].Value == null ||
                                    pck.Workbook.Worksheets[0].Cells[i, 2].Value == null) continue;

                                var patientDescription = new PatientDescription()
                                {
                                    Patient = new Patient()
                                };
                                patientDescription.Patient.First = (string)pck.Workbook.Worksheets[0].Cells[i, 1].Value;
                                patientDescription.Patient.Last = (string)pck.Workbook.Worksheets[0].Cells[i, 2].Value;
                                patientDescription.Patient.FullName = patientDescription.Patient.First + " " + patientDescription.Patient.Last;
                                if (pck.Workbook.Worksheets[0].Cells[i, 3].Value != null)
                                {
                                    if (pck.Workbook.Worksheets[0].Cells[i, 3].Value.GetType() == typeof(DateTime))
                                    {
                                        patientDescription.Patient.DOB = (DateTime)pck.Workbook.Worksheets[0].Cells[i, 3].Value;
                                    }
                                    else if (pck.Workbook.Worksheets[0].Cells[i, 3].Value.GetType() == typeof(string))
                                    {
                                        var re = new Regex(@"(\d{1,2})[/-](\d{1,2})");
                                        var m = re.Match((string)pck.Workbook.Worksheets[0].Cells[i, 3].Value);
                                        if (m.Success)
                                        {
                                ...
                                patientDescription.Patient.MRN = Convert.ToString(pck.Workbook.Worksheets[0].Cells[i, 4].Value);
                                patientDescription.Patient.PatientId = (string)pck.Workbook.Worksheets[0].Cells[i, 5].Value;
                                if (patientDescription.Patient.PatientId == null)
                                {
                                    patientDescription.Patient.PatientId = patientDescription.Patient.MRN;
                                }
                                var allergiesString = (string)pck.Workbook.Worksheets[0].Cells[i, 6].Value;
                    ...
                    if (pck.Workbook.Worksheets[1].Dimension.Rows > 1)
                    {
                        ...
                        for (int i = pck.Workbook.Worksheets[1].Dimension.Start.Row + 1;
                                 ...
                                 i++)
                        {
                            if (pck.Workbook.Worksheets[1].Cells[i, 6].Value == null) continue;
                            PatientDescription patientDescription = null;
                            var patientIdList = new List<string>();
                            var patientIdIndex = 6;
                            do
                            {
                                if (patientIdIndex > pck.Workbook.Worksheets[1].Dimension.End.Column ||
                                    pck.Workbook.Worksheets[1].Cells[i, patientIdIndex].Value == null) break;
                                var patientId = Convert.ToString(pck.Workbook.Worksheets[1].Cells[i, patientIdIndex].Value);
                    ...
                    if (pck.Workbook.Worksheets[2].Dimension.Rows > 1)
                    {
                                 ...
                                 i++)
                        {
                            ...
                            {
                                _id = -1,
                                GenericName = (string)pck.Workbook.Worksheets[2].Cells[i, 1].Value,
                                Alias = (string)pck.Workbook.Worksheets[2].Cells[i, 2].Value,
                                Strength = pck.Workbook.Worksheets[2].Cells[i, 3].Value == null ? string.Empty : pck.Workbook.Worksheets[2].Cells[i, 3].Value.GetType() == typeof(string) ? (string)pck.Workbook.Worksheets[2].Cells[i, 3].Value : Convert.ToString(pck.Workbook.Worksheets[2].Cells[i, 3].Value),
                                StrengthUnit = (string)pck.Workbook.Worksheets[2].Cells[i, 4].Value,
                                Volume = pck.Workbook.Worksheets[2].Cells[i, 5].Value == null ? string.Empty : pck.Workbook.Worksheets[2].Cells[i, 5].Value.GetType() == typeof(string) ? (string)pck.Workbook.Worksheets[2].Cells[i, 5].Value : Convert.ToString(pck.Workbook.Worksheets[2].Cells[i, 5].Value),
                                VolumeUnit = (string)pck.Workbook.Worksheets[2].Cells[i, 6].Value,
                                TotalContainerVolume = (string)pck.Workbook.Worksheets[2].Cells[i, 7].Value,
                                Route = (string)pck.Workbook.Worksheets[2].Cells[i, 8].Value,
                            });
                        }
                    }
